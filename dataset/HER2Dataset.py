...
class HER2Dataset(torch.utils.data.Dataset):
    def __init__(self, path='/newdata/why/HER2/',mode='train',selected_genes=None):
        # selected_genes=top_gene_selection(path,2000)
        if selected_genes==None:
            selected_genes=np.load('Breast_Result_Gene_HER2.npy',allow_pickle=True).tolist()
        ...
        wsi_filenames=sorted(glob.glob(path+'/images/HE/*.jpg'))
        random.seed(1553)
        ...
        for wsi_file in wsi_filenames:
            ...
            wsi_basename=os.path.basename(wsi_file).split(r'.')[0]
            # st_coords=path+'/spot-selections/'+wsi_basename+'_selection.tsv' # stores label, useless in SR
            st_feats=path+'/count-matrices/'+wsi_basename+'.tsv'
            st_spots_pixel_map=path+'/spot-selections/'+wsi_basename+'_selection.tsv'
